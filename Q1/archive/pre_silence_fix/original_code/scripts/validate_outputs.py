...
for sid in sorted(expected):
    ...
    for mod,dim in [('text',768),('audio',27),('vision',15)]:
        if mod=='text':
            ...
            if mod=='audio':duration=(media['audio_samples']-media['audio_gap_samples'])/16000
            else:
...
ordered=sorted(records,key=lambda r:r['media']['container_duration_s'])
selected=[ordered[i]['sample_id'] for i in np.linspace(0,99,10).round().astype(int)]
review=[]
for sid in selected:
