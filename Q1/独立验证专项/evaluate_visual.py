...
PROTOCOL = dict(outer_folds=5, inner_folds=4, alphas=[0.1,1,10,100,1000],
                ...
                primary='vision15 versus training-median baseline',
...
def main():
    p=argparse.ArgumentParser(); p.add_argument('--features',type=Path,required=True); p.add_argument('--labels',type=Path,required=True); p.add_argument('--out',type=Path,required=True); a=p.parse_args()
    ...
    blocks={k:[] for k in ['quality','framing','shape','vision15','text','text_vision']}; hashes={}; zero=[]
    ...
    contrasts=[]
    for candidate,reference in [('vision15','median_baseline'),('shape','median_baseline'),('vision15','framing'),('shape','quality'),('text_vision','text')]:
    ...
    metadata=dict(n=len(y),source_videos=len(unique),zero_detection_samples=zero,features_sha256=hashes,labels_sha256=sha(a.labels),script_sha256=sha(__file__),python=sys.version,platform=platform.platform(),sklearn=sklearn.__version__,numpy=np.__version__,all_group_disjoint=True,all_predictions_out_of_fold=True)
