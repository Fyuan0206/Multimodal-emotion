...
files += [(f,'figures/'+f.name) for f in args.figures.iterdir() if f.is_file() and f.suffix in ['.png','.eps','.pdf','.svg','.json','.csv','.xlsx','.md']]
...
for name in ['silence-fix-1529268.log','final-audit-1529289.log','server_plots.log','server_tests.log','server_validation.log','server_reproducibility.log','server_run.log','tests_migration_local.log','server_figure_audit.json','nature_source_audit.json','tex_unchanged_migration.json']:
    f=ROOT/'logs'/name
    if f.is_file(): files.append((f,'logs/'+name))
# Include supplementary evidence and actual review aids, not pretrained weights or archives.
for directory in ['独立验证专项','视觉增强专项','人工回听专项','有声回听专项','模型辅助验证','模型辅助验证_KimiK3','模型辅助验证_GPT6Pro']:
    for f in (ROOT/directory).rglob('*'):
        if f.is_file() and not {'__pycache__','.pytest_cache','models'} & set(f.relative_to(ROOT/directory).parts) and f.suffix not in ['.mp4','.pyc']:
            files.append((f,f.relative_to(ROOT).as_posix()))
# Preserve frozen diagnostic input provenance without bundling old archives/weights.
old=ROOT/'archive/pre_silence_fix'
for folder in ['outputs_server_a100/features','original_code']:
    for f in (old/folder).rglob('*'):
        if f.is_file() and '__pycache__' not in f.parts:
            files.append((f,f.relative_to(ROOT).as_posix()))
for filename in ['environment.json','manifest.csv','validation.json','cross_platform_comparison.json','cross_platform_boundary_changes.csv']:
    f=old/'outputs_server_a100'/filename
    if f.exists(): files.append((f,f.relative_to(ROOT).as_posix()))
for name in ['FINAL_STATUS.md']:
    f=ROOT/name
    if f.exists(): files.append((f,name))
