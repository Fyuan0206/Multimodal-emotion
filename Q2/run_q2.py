...
def predict(model, features, batch_size=512):
    ...
    with torch.inference_mode():
        for start in range(0, len(features["text"]), batch_size):
            ...
            inputs = [torch.from_numpy(features[key][start:end]) for key in (*MODES, "coverage")]
            reg, cls = model(*inputs)
            predictions.append(reg.numpy())
            logits.append(cls.numpy())
...
def fit_variant(train_features, y_reg, y_cls, valid_features, valid_stress, valid_reg, valid_cls,
                learned_gate, seed, epochs=30, patience=6):
    torch.manual_seed(seed)
    model = RobustFusion(learned_gate=learned_gate)
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    counts = np.bincount(y_cls, minlength=3)
    class_weights = torch.as_tensor(np.sqrt(counts.sum() / (3 * counts)), dtype=torch.float32)
    features = {key: torch.from_numpy(value) for key, value in train_features.items()}
    targets_reg = torch.from_numpy(y_reg)
    targets_cls = torch.from_numpy(y_cls)
    rng = np.random.default_rng(seed)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for indices in np.array_split(rng.permutation(len(y_reg)), max(1, int(np.ceil(len(y_reg) / 128)))):
            reg, cls = model(*(features[key][indices] for key in (*MODES, "coverage")))
            loss = nn.functional.smooth_l1_loss(reg, targets_reg[indices], beta=0.5)
            loss = loss + 0.7 * nn.functional.cross_entropy(cls, targets_cls[indices], weight=class_weights)
...
def encode_attachment3(data_dir, norms):
    ...
    bert = BertModel.from_pretrained(str(model_path), local_files_only=True).eval()
    bert_outputs = []
    with torch.inference_mode():
        for start in range(0, len(samples), 8):
            batch = bert_inputs[start:start + 8]
            encoded = bert(
                input_ids=torch.from_numpy(batch[:, 0, :]),
                attention_mask=torch.from_numpy(batch[:, 1, :]),
                token_type_ids=torch.from_numpy(batch[:, 2, :]),
            ).last_hidden_state
            bert_outputs.append(encoded.numpy())
    sample_split = {
...
def predict_attachment3(data_dir, output, norms, model, neutral_bias):
    paths, test = encode_attachment3(data_dir, norms)
...
def main():
    ...
    for index, (name, spec) in enumerate(variants.items()):
        ...
        model, fit = fit_variant(fit_features, y_reg, y_cls, eval_clean, eval_stress,
                                 valid["y_reg"], valid["y_cls"], gate, SEED + index, epochs=epochs)
    ...
    paths = predict_attachment3(attachment3, args.output, norms, selected_model, selected_bias)
    run = {
        "seed": SEED, "data_version": "aligned_50", "train_n": len(train["y_reg"]),
